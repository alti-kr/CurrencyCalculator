        ...
        return delegate.enqueue(object : Callback<S> {
            override fun onResponse(call: Call<S>, response: Response<S>) {
                ...
                if (response.isSuccessful) {
                    ...
                    callback.onResponse(
                        this@NetworkResponseCall,
                        Response.success(NetworkResponse.Error(NetworkResponse.RetrofitException.httpError(response)))
                    )
                }
            }
            ...
            override fun onFailure(call: Call<S>, throwable: Throwable) {
                val networkResponse = when (throwable) {
                    is IOException -> NetworkResponse.Error(NetworkResponse.RetrofitException.networkError(throwable))
                    else -> NetworkResponse.Error(NetworkResponse.RetrofitException.unexpectedError(throwable))
                }
